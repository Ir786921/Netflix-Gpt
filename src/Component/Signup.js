...
const Signup = () => {
  ...
  function LoginClicked() {
    const text = validation(email?.current?.value,password?.current?.value,fname?.current?.value);
  ...
  return (
    <div className=" m-auto p-4 bg-black opacity-85 rounded-md shadow-md w-[450px]">
      <form onSubmit={(e)=>{
           e.preventDefault();   
      }}  action="" className="  flex justify-center flex-col ">
        ...
        />
        <br />
        <p className=" text-[#E50914] text-lg font-semibold ml-10">{validationText}</p>
        <br />

        <button className="  w-[350px] m-auto p-2 rounded-md shadow-sm text-white bg-[#E50914]" onClick={LoginClicked}>
          {isclick ? "SignUp" : "Login"}
        </button>
      </form>
      <br />
